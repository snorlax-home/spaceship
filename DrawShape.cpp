...
void DrawShape::Circle(float x, float y, float radius, int rotate, int type, bool smoothing, int resolution, DWORD color)
{
...
void DrawShape::CircleFilled(float x, float y, float rad, float rotate, int type, int resolution, DWORD color)
{
	std::vector<vertex> circle(resolution + 2);
	float angle = rotate * D3DX_PI / 180;
	float pi;
...
void DrawShape::BoxBordered(float x, float y, float w, float h, float border_width, DWORD color, DWORD color_border)
{
	BoxFilled(x, y, w, h, color);
	Box(x - border_width, y - border_width, w + 2 * border_width, h + border_width, border_width, color_border);
}

void DrawShape::BoxFilled(float x, float y, float w, float h, DWORD color)
{
	vertex V[4];
...
void DrawShape::Message(char* text, float x, float y, int font, int orientation)
{
	RECT rect = { x, y, x, y };
...
void DrawShape::Sprite(LPDIRECT3DTEXTURE9 tex, float x, float y, float resolution, float scale, float rotation)
{
...
void DrawShape::AddFont(char* Caption, float size, bool bold, bool italic)
{
